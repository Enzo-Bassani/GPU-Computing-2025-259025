...
def plot_and_save_bar(programs, data, title, ylabel, filename, color=None, log_scale=False, cache_misses=None):
    ...
    # Add cache_miss data if available
    if cache_misses and any(x is not None for x in cache_misses):
        try:
            ...
            for x, miss, prog in zip(x_positions, cache_misses, programs):
                if miss is not None:
                    try:
                        ...
                        # Convert to percentage if not already (assume it's a percentage if > 1)
                        miss_pct = miss_float
            ...
            if valid_positions:
                ...
                ax2.set_ylabel('Cache Misses (%)', color='r', fontsize=14)
                ax2.tick_params(axis='y', labelcolor='r')

                # Add legend for cache misses
                ax2.legend(loc='upper left')

                # Add percentage labels above the points
                for x, y, prog in zip(valid_positions, valid_misses, program_labels):
                    ax2.annotate(f'{y:.2f}%',
                                (x, y),
                                xytext=(0, 10),  # Increase vertical offset for better visibility
